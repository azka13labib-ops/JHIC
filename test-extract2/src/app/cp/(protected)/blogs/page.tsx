...
import { FolderOpen, Edit, Calendar, ExternalLink } from 'lucide-react';
...
export default function AdminBlogsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/admin/blogs`, { headers: authHeaders });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 shadow-xs overflow-hidden">
        ...
        <AdminSearchBar
          ...
          placeholder="Cari judul/pemilik atau tautan blog..."
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead className="bg-slate-50 text-slate-600 border-b border-slate-200 uppercase font-bold tracking-wider">
                <tr>
                  <th className="py-4 px-6">Nama / Judul Blog</th>
                ...
                {paginatedBlogs.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="py-12 text-center text-slate-400">
                      {search ? 'Tidak ada blog yang cocok dengan pencarian.' : 'Belum ada blog siswa yang terdaftar.'}
                    </td>
                  </tr>
                ) : (
