...
export function StudentWorkForm({ isEdit = false, initialData }: StudentWorkFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = isEdit ? `/admin/student-works/${initialData?.id}` : `/admin/student-works`;
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">
              {isEdit ? 'Edit Karya Siswa' : 'Upload Karya Siswa'}
            </h1>
            <p className="text-xs text-slate-500 mt-0.5">
              {isEdit ? 'Perbarui informasi dan foto dokumentasi karya siswa.' : 'Pamerkan portofolio seni, teknologi, dan inovasi siswa.'}
            </p>
          </div>
        </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  <span>{isEdit ? 'Menyimpan...' : 'Memublikasikan...'}</span>
                </>
              ) : (
                <>
                  <Save className="w-4 h-4" />
                  <span>{isEdit ? 'Simpan Perubahan' : 'Publikasikan Karya'}</span>
                </>
              )}
            </button>
          </div>
