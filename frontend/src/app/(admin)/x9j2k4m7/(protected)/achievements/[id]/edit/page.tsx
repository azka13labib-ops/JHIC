...
export default function AdminEditAchievementPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Gagal memuat data prestasi');
    } finally {
      setFetching(false);
    }
  }, [id, session]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push('/x9j2k4m7/achievements');
        router.refresh();
      } else {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.message || 'Gagal mengupdate prestasi');
      }
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Terjadi kesalahan saat mengupdate');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3">
        ...
        <span className="text-xs font-semibold text-slate-400">Memuat data prestasi...</span>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Edit Prestasi Siswa</h1>
            <p className="text-xs text-slate-500 mt-0.5">Perbarui informasi kejuaraan, tingkat lomba, dan dokumentasi piala.</p>
          </div>
        </div>
