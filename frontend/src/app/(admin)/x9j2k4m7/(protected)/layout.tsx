...
export default function ProtectedAdminLayout({
  ...
  useEffect(() => {
    ...
    if (status === 'unauthenticated') {
      window.location.replace('/x9j2k4m7/login');
    } else if (status === 'authenticated' && session?.user?.role !== 'admin') {
      window.location.replace('/x9j2k4m7/login?error=AccessDenied');
    }
  }, [status, session]);
  ...
  const handleConfirmLogout = async () => {
    try {
      ...
      // Use the correct login route and force a hard redirect
  ...
  return (
    <div className="admin-root flex min-h-screen bg-[#f8fafc] text-slate-900 font-sans" style={{ colorScheme: 'light' }}>
      ...
      <aside className="hidden lg:flex w-72 flex-col border-r border-slate-200/80 bg-white text-slate-900 shrink-0 select-none">
        ...
        <div className="h-20 flex items-center gap-3 px-6 border-b border-slate-100 bg-white">
          <div className="w-11 h-11 rounded-xl bg-slate-50 border border-slate-200 p-1.5 flex items-center justify-center shadow-xs">
            <Image
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        {mobileOpen && (
          <div className="lg:hidden fixed inset-0 z-50 bg-black/50 backdrop-blur-xs flex">
            <div className="w-72 bg-white h-full flex flex-col shadow-2xl p-5 space-y-6 overflow-y-auto">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                <div className="flex items-center gap-2.5">
                  <Image src="/logo-sekolah.jpg" alt="Logo" width={32} height={32} />
                  <span className="font-bold text-sm">SMA PGRI 1</span>
                </div>
                <button onClick={() => setMobileOpen(false)} className="p-1 rounded text-slate-500 cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2.5 text-xs font-bold text-rose-600 bg-rose-50 rounded-xl cursor-pointer"
                >
