  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Manajemen Blog Siswa</h1>
          <p className="text-muted-foreground">Kelola berita dan pengumuman sekolah.</p>
        </div>
        <Link href="/admin/blogs/create">
          <Button>
            <Plus className="mr-2 h-4 w-4" /> Tambah Blog Siswa
          </Button>
        </Link>
      </div>
              ...
              blogsList.map((item: any) => (
                <tr key={item.id} className="bg-white border-b hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right space-x-2">
                    ...
                    <DeleteBlogButton id={item.id} title={item.title} />
                  </td>
                </tr>
              ))
            )}
