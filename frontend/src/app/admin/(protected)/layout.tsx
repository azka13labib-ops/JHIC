...
export default function ProtectedAdminLayout({
  ...
  const [navGroups, setNavGroups] = useState(navigationGroups);

  useEffect(() => {
    if (status === 'authenticated') {
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000/api'}/ppdb/info`)
        .then((res) => res.json())
        .then((json) => {
          if (json?.data?.academic_year) {
            const year = json.data.academic_year.split('/')[0];
            setNavGroups((prev) => 
              prev.map(g => ({
                ...g,
                items: g.items.map(item => 
                  item.href === '/admin/ppdb' 
                    ? { ...item, badge: `PPDB ${year}` }
                    : item
                )
              }))
            );
    ...
    <div className="admin-root flex h-screen bg-[#f8fafc] text-slate-900 font-sans" style={{ colorScheme: 'light' }}>
              ...
              {grp.items.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`relative flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden group ${
                      isActive
                        ? 'bg-blue-50/80 text-blue-700 shadow-sm'
                        : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100/80'
                    }`}
                  >
                    {/* Active State Sliding Indicator */}
                    <div className={`absolute left-0 top-0 bottom-0 w-1 bg-blue-600 rounded-r-full transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] origin-left ${isActive ? 'scale-x-100' : 'scale-x-0'}`} />
                    
                    <div className="flex items-center gap-3 relative z-10">
                      <Icon className={`w-4 h-4 transition-all duration-300 ease-out group-hover:scale-110 group-hover:translate-x-0.5 ${isActive ? 'text-blue-600' : 'text-slate-400 group-hover:text-blue-600'}`} />
                      <span>{item.title}</span>
                    </div>
                    {item.badge && (
                      <span className={`relative z-10 text-[9px] px-2 py-0.5 rounded-full font-bold uppercase transition-colors duration-300 ${
                        isActive ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-slate-100 text-slate-500 border border-slate-200 group-hover:bg-blue-50 group-hover:text-blue-600'
                      }`}>
                        {item.badge}
                      </span>
                    )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 border-b border-slate-200/80 bg-white px-4 sm:px-8 flex items-center justify-between gap-4 sticky top-0 z-30 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              className="inline-flex items-center justify-center gap-2 w-9 h-9 sm:w-auto sm:h-auto sm:px-3.5 sm:py-1.5 rounded-xl border border-slate-200 text-slate-600 hover:text-blue-600 hover:border-blue-300 bg-slate-50 hover:bg-white text-xs font-bold transition-all shadow-xs group"
              title="Lihat Website"
            >
              <span className="hidden sm:inline">Lihat Website</span>
              <ExternalLink className="w-4 h-4 sm:w-3.5 sm:h-3.5 text-slate-400 group-hover:text-blue-600 transition-colors" />
            </a>
                  ...
                  {grp.items.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        href={item.href}
                        onClick={() => setMobileOpen(false)}
                        className={`relative flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] overflow-hidden group ${
                          isActive ? 'bg-blue-50/80 text-blue-700 shadow-sm' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100/80'
                        }`}
                      >
                        {/* Active State Sliding Indicator */}
                        <div className={`absolute left-0 top-0 bottom-0 w-1 bg-blue-600 rounded-r-full transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] origin-left ${isActive ? 'scale-x-100' : 'scale-x-0'}`} />
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          <div key={pathname} className="max-w-7xl mx-auto animate-fade-in-up">
            {children}
          </div>
        </main>
