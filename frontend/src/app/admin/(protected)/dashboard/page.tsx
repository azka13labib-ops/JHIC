...
export default function DashboardPage() {
  ...
  const quickActions = [
    { label: 'Tulis Berita Baru', href: '/admin/news/create', icon: Newspaper },
    { label: 'Tambah Agenda', href: '/admin/agendas/create', icon: Calendar },
    { label: 'Input Prestasi', href: '/admin/achievements/new', icon: Trophy },
    { label: 'Upload Karya Siswa', href: '/admin/student-works/create', icon: PenTool },
    { label: 'Tambah Galeri Foto', href: '/admin/galleries/create', icon: ImageIcon },
    { label: 'Tambah Artikel', href: '/admin/articles/create', icon: BookOpen },
  ];
      ...
      <div className="rounded-xl bg-white border border-slate-200 p-5 sm:p-6 shadow-2xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 shrink-0">
            <Link
              href="/admin/ppdb"
              className="justify-center px-4 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white text-xs font-bold rounded-lg shadow-2xs transition-colors flex items-center gap-1.5"
            >
            ...
            <Link
              ...
              className="justify-center px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 text-xs font-bold rounded-lg transition-colors flex items-center gap-1.5"
            >
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs space-y-4">
            ...
            {loading ? (
              ...
              <div className="-mx-5 sm:mx-0">
                ...
                <div className="sm:hidden flex flex-col divide-y divide-slate-100 border-y border-slate-100">
                  {stats.recent_registrations.map((reg) => {
                    const statusObj = STATUS_CONFIG[reg.status as keyof typeof STATUS_CONFIG] || {
                      label: reg.status,
                      badge: 'bg-slate-100 text-slate-700 border-slate-200',
                    ...
                    return (
                      <div key={reg.id} className="p-4 hover:bg-slate-50 transition-colors flex flex-col gap-2.5">
                        <div className="flex justify-between items-start gap-3">
                          <div className="font-bold text-slate-900 text-sm leading-tight">{reg.full_name}</div>
                          <span className={`shrink-0 px-2 py-0.5 rounded text-[10px] font-bold border ${statusObj.badge}`}>
                            {statusObj.label}
                          </span>
                        </div>
                        <div className="flex justify-between items-center text-xs">
                          <div className="font-mono font-semibold text-blue-700">{reg.registration_number}</div>
                          <div className="text-slate-500 font-medium">{reg.major_choice || 'Umum'}</div>
                        </div>
                      </div>
                    );
                  })}
